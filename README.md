...
> Status: beta. `v0.1.5` is the latest release; expect rough edges. Product overview: [simplycubed.com/code](https://simplycubed.com/code?utm_source=github&utm_medium=readme&utm_campaign=code). See [Status](#status).
...
```sh
go install github.com/simplycubed/code/cmd/simplycubed@v0.1.5
simplycubed version
```

That prints `0.1.5`. Pre-1.0 releases follow semver with the usual caveat: minor
...
Beta, and honest about it. Two loops run end to end via the CLI on the Codex-on-Azure engine: issue to pull request, and fix-on-request (a human requests changes, the fixer addresses them and pushes back). `v0.1.5` is the latest release and you should still expect rough edges.
